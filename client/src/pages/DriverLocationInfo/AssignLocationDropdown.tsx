...
function getDropoffLocationsUIStateList(
  ...
  const idsOfDropoffLocationsAssignedToThisDriver = new Set(driver.dropoffLocations);
...
function updateDropoffLocationsUIStateList(prevUIStateList: DropoffLocationUIState[], idx: number, field: keyof DropoffLocationUIState, newState: boolean) {
...
> = ({
  ...
  driver,
    ...
    const [dropoffLocationsUIStateList, setDropoffLocationsUIStateList] = useState<DropoffLocationUIState[]>(getDropoffLocationsUIStateList(locations, driver));
    ...
    const assignLocationMutation = useMutation({
      mutationFn: async ({ id, idx }: { id: string, idx: number }) => {
        ...
        const locationIds = dropoffLocationsUIStateList[idx].isSelected ? [] : [id];
        ...
        setDropoffLocationsUIStateList((prev) => updateDropoffLocationsUIStateList(prev, idx, "isLoading", true));
      ...
      onSuccess: (data, { id, idx }) => {
        ...
        const uiStateWithUpdatedLoading = updateDropoffLocationsUIStateList(dropoffLocationsUIStateList, idx, "isLoading", false);
        //optimistically update selection state 
        const uiStateWithUpdatedLoadingAndSelection = updateDropoffLocationsUIStateList(uiStateWithUpdatedLoading, idx, "isSelected", !uiStateWithUpdatedLoading[idx].isSelected);
        ...
        toastNotify(
          `Location ${uiStateWithUpdatedLoadingAndSelection[idx].isSelected ? "assigned" : "unassigned"
          }`,
          "success"
        );
      },
      onError: (error, { id, idx }) => {
        ...
        setDropoffLocationsUIStateList((prev) => updateDropoffLocationsUIStateList(prev, idx, "isLoading", false));
          ...
          >
            {locations.map((item, i) => (
              <AssignLocationDropdownItem
                ...
                onAssign={() => assignLocationMutation.mutate({ id: item.id, idx: i })}
              />
            ))}
          </DropdownMenu.Content>
        </DropdownMenu.Portal>
