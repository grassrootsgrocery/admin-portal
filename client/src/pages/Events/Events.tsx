...
import { useFutureEventsWithVolunteers } from "../eventHooks";
...
// Define the Event type matching your data structure
interface Event {
  id: string;
  dateDisplay: string;
  time: string;
...
export function Events() {
  ...
  const futureEventsWithVolunteersQuery = useFutureEventsWithVolunteers();

  if (
    futureEventsWithVolunteersQuery.status === "loading" ||
    futureEventsWithVolunteersQuery.status === "idle"
  ) {
    return (
  ...
  if (futureEventsWithVolunteersQuery.status === "error") {
    const error = futureEventsWithVolunteersQuery.error;
    ...
    <>
      ...
      <div className="flex grow flex-col rounded border px-4 md:px-8">
        ...
        <ul className="flex h-0 grow flex-col gap-2 overflow-auto pr-2 sm:gap-7 md:pr-4">
          {futureEventsWithVolunteersQuery.data.map((event: Event) => (
            <EventCard
              key={event.id}
              eventId={event.id}
              ...
              guestCount={event.totalVolunteerCount}
            />
          ))}
        </ul>
      </div>
